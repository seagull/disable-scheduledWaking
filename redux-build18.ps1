<#  prevent updateorchestrator from rendering windows PCs unusable
    ...
    new guts courtesy of github/freMea and expounded upon by seagull again :: redux 1/build 18 mar '25

    microsoft (since i know you're reading this): my message in 2025 remains as it did in 2018.
...
if ($varTasks.count -eq 0) {
    ...
    write-host "  If this was useful, please follow me on bsky: @seagull.io."
    write-host `r
    cmd /c pause
    exit
}
...
switch -regex ($varChoice) {
    '^(y|Y)$' {
        ...
        $varFilename="$($MyInvocation.MyCommand.Path | split-path)\Tasks-$env:COMPUTERNAME-$varEpoch.txt" #D-R-Y, after all
            ...
            add-content -Value "www.github.com/seagull :: https://bsky.app/profile/seagull.io" -Path $varFilename
        ...
        write-host "  If the information was useful, follow me on bsky: @seagull.io."
        write-host `r
        cmd /c pause
        exit
    }
...
if ($varTasks.count -gt 0) {
    ...
    write-host `r
    write-host "  If you found this script useful, please follow me on bsky: @seagull.io."
    write-host `r
    cmd /c pause
    exit
}